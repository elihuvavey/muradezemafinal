...
class LoggingAudioHandler extends CompositeAudioHandler {
  LoggingAudioHandler(super.inner) {
    playbackState.listen((state) {
      debugPrint('playbackState changed: $state');
    });
    queue.listen((queue) {
      debugPrint('queue changed: $queue');
    });
    queueTitle.listen((queueTitle) {
      debugPrint('queueTitle changed: $queueTitle');
    });
    mediaItem.listen((mediaItem) {
      debugPrint('mediaItem changed: $mediaItem');
    });
    ratingStyle.listen((ratingStyle) {
      debugPrint('ratingStyle changed: $ratingStyle');
    });
    androidPlaybackInfo.listen((androidPlaybackInfo) {
      debugPrint('androidPlaybackInfo changed: $androidPlaybackInfo');
    });
    customEvent.listen((dynamic customEventStream) {
      debugPrint('customEvent changed: $customEventStream');
    });
    customState.listen((dynamic customState) {
      debugPrint('customState changed: $customState');
    });
  }
